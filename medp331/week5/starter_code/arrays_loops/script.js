...
let row2 = document.querySelectorAll('.box-row-1')
let row3 = document.querySelectorAll('.box-row-1')
...
function changeBoxColors(elements, colors) {
    for(let i = 0; i < elements.length; i++)
    {
        ...
        {
            S(elements[i][i].backgroundColor = colors[i][j]);
...
{
    console.log('while loop counter', whileLoopCounter)
    whileLoopCounter++;
}